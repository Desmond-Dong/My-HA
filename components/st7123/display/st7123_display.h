...
constexpr static const char *const TAG = "display.st7123";

class ST7123Display : public display::Display {
 public:
  ST7123Display(size_t width, size_t height, display::ColorBitness color_depth, uint8_t pixel_mode)
 ...
 protected:
  ...
  void write_to_display_(int x_start, int y_start, int w, int h, const uint8_t *ptr, int x_offset, int y_offset,
                         int x_pad);
  bool check_buffer_();
  bool setup_panel_wrapper_(esp_lcd_panel_handle_t panel);
  ...
  uint16_t vsync_front_porch_ = 220;
  float pclk_frequency_ = 70;
  float lane_bit_rate_{1000};
  uint8_t lanes_{2};
  bool invert_colors_{};
