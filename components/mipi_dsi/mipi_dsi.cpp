...
/** Reusable 64-byte-aligned line buffer for RGB565 byte swap (SPIRAM preferred; internal DRAM fallback). */
static uint8_t *ensure_rgb565_line_buf(size_t line_bytes) {
  if (rgb565_line_swap_cap_ >= line_bytes && rgb565_line_swap_buf_ != nullptr)
  ...
  rgb565_line_swap_buf_ = static_cast<uint8_t *>(
      heap_caps_aligned_alloc(RGB565_LINE_BUF_ALIGN, line_bytes, MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT));
  if (rgb565_line_swap_buf_ != nullptr) {
    rgb565_line_swap_cap_ = line_bytes;
    return rgb565_line_swap_buf_;
  }
  rgb565_line_swap_buf_ =
      static_cast<uint8_t *>(heap_caps_aligned_alloc(RGB565_LINE_BUF_ALIGN, line_bytes, MALLOC_CAP_8BIT));
  if (rgb565_line_swap_buf_ != nullptr)
    rgb565_line_swap_cap_ = line_bytes;
  return rgb565_line_swap_buf_;
}
...
void MIPI_DSI::write_to_display_(int x_start, int y_start, int w, int h, const uint8_t *ptr, int x_offset, int y_offset,
  ...
  if (this->color_depth_ == display::COLOR_BITNESS_565) {
    ...
    if (line_buf != nullptr) {
      // Tab5 V2 RGB565 diagnostic aligned line byte-swap test.
      for (int y = 0; y != h; y++) {
    ...
    ESP_LOGE(TAG, "MIPI_DSI: RGB565 line-swap buffer alloc failed (need %u bytes); falling back to direct draw",
             (unsigned) line_bytes);
  }
