...
from .. import st7121_ns

DEPENDENCIES = ["esp32", "esp_ldo", "psram"]

ST7121Display = st7121_ns.class_("ST7121Display", display.Display, cg.Component)
...
CONFIG_SCHEMA = cv.All(
    display.FULL_DISPLAY_SCHEMA.extend(
        {
            cv.GenerateID(): cv.declare_id(ST7121Display),
            cv.Optional(CONF_DIMENSIONS, default={"width": 720, "height": 1560}): dimension_schema(1),
            cv.Optional(CONF_RESET_PIN, default={"pi4ioe5v6408": "pi4ioe1", "number": 4}): pins.gpio_output_pin_schema,
            cv.Optional(CONF_ENABLE_PIN, default=[]): cv.ensure_list(pins.gpio_output_pin_schema),
            ...
            cv.Optional(CONF_PCLK_FREQUENCY, default="78MHz"): cv.All(cv.frequency, cv.Range(min=4e6, max=100e6)),
            cv.Optional(CONF_LANES, default=2): cv.int_range(1, 2),
            cv.Optional(CONF_LANE_BIT_RATE, default="1300Mbps"): cv.All(cv.bps, cv.Range(min=100e6, max=3200e6)),
            ...
            cv.Optional(CONF_VSYNC_FRONT_PORCH, default=320): cv.int_,
        }
    ),
    cv.only_on_esp32,
